...
process.on('message', async (message) => {
  ...
  try {
    ...
    const inputName = path.join(__dirname, "html", `${jobId}.html`);
    const outputName = path.join(__dirname, "pdfs", `${jobId}.pdf`);
    ...
    exec(command, (error, stdout, stderr) => {
      ...
      if (error) {
        ...
        process.send({ type: 'error', error: "Something went wrong with generating the PDF." });
        return;
      }
